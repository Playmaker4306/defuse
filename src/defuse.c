...
int main(int argc, char* argv[])
{
    ...
    if (output_fd == -1)
        FATAL("Error opening file \"%s\" for writing.", output);
    ...
    for (int i = 1; i <= NUM_PHASE; i++)
    {
        ...
        {
            bytes_written = write(output_fd, bomb.answer[i], strlen(bomb.answer[i]));
            if (bytes_written == -1) {
                perror("Failed to write answer");
                close(output_fd);
                exit(EXIT_FAILURE);
            }
        }
        ...
        {
            bytes_written = write(output_fd, "<Answer not found>", 19);
            if (bytes_written == -1) {
                perror("Failed to write default message");
                close(output_fd);
                exit(EXIT_FAILURE);
            }
        }
